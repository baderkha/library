...
func (c *CrudGorm[t]) GetWithFilterExpression(f *rql.FilterExpression, s *rql.SortExpression) (data []*t, err error) {
	...
	sql := fmt.Sprintf("SELECT * FROM %s WHERE 1=1 %s %s", c.Table, out.Query, outSort.RawQuery)
...
func (c *CrudGorm[t]) GetWithFilterExpressionPaginated(f *rql.FilterExpression, p *rql.PaginationExpression, s *rql.SortExpression) (data *Paginated[t], err error) {
	...
	go func() {
		...
		sql := fmt.Sprintf("SELECT * FROM %s WHERE 1=1 %s %s %s", c.Table, out.Query, outSort.RawQuery, limitClause)
	...
	go func() {
		...
		sql := fmt.Sprintf("SELECT COUNT(*) FROM %s WHERE 1=1 %s %s", c.Table, out.Query, outSort.RawQuery)
